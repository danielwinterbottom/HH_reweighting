...
#os.system('cd %(current_dir)s/%(input)s; { echo 0; echo set nevents 100; echo set gridpack True; } | python2 bin/generate_events %(name)s; cd %(current_dir)s' % vars())
...
tar -xvf %(current_dir)s/%(input)s/%(name)s_gridpack.tar.gz\n\
...
for i in range(1,args.total_events/args.events_per_job+1):
    ...
    #out_string += 'if [ $SGE_TASK_ID == %(i)i ]; then { echo 0; echo set run_card iseed %(i)i; echo set nevents %(nevents)i; } | python2 bin/generate_events parajob_output_%(i)i &> %(current_dir)s/jobs/parajob_%(name)s_%(i)i.log; fi\n' % vars()
#    out_string += 'if [ $SGE_TASK_ID == %(i)i ]; then\n\
#\tcd %(current_dir)s/batch_job_outputs/%(name)s/job_output_%(i)i\n\
#\trm -rf %(current_dir)s/batch_job_outputs/%(name)s/job_output_%(i)i/*\n\
#\tcp %(current_dir)s/%(input)s/%(name)s_gridpack.tar.gz .\n\
#\ttar -xvf %(name)s_gridpack.tar.gz\n\
#\t./run.sh %(nevents)i %(i)i\n\
#\tmv events.lhe.gz events_%(i)i.lhe.gz\n\
#\techo 0 | madevent/bin/madevent reweight events_%(i)i.lhe.gz\n\
#\trm -rf madevent run.sh %(name)s_gridpack.tar.gz py.py\n\
#fi\n' % vars()
#\t./run.sh %(nevents)i %(i)i &> run.log\n\
#\techo 0 | madevent/bin/madevent reweight events_%(i)i.lhe.gz &> reweight.log\n\
...
os.system('qsub -q hep.q -l h_rt=300 -t 1-%(total)i:1 jobs/parajob_%(name)s.sh' % vars())
#os.system('qsub -e /dev/null -o /dev/null -q hep.q -l h_rt=300 -t 1-%(total)i:1 jobs/parajob_%(name)s.sh' % vars())
